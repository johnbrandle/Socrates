 ...
 * 
 * For full license text of the original code, refer to the LICENSE file or https://github.com/nayuki/QR-Code-generator
 */
...
@SealedDecorator()
export class QRCodeUtil<A extends IBaseApp<A>>
{
    private _app:A;
    ...
    public create(text:string, color:boolean=true):HTMLCanvasElement
    {
        const qrCode = qrcodegen.QrCode.encodeText(text, qrcodegen.QrCode.Ecc.HIGH);
     
        return this.drawCanvas(qrCode, 12, 1, '#ffffff', color ? '#000000' : '#222222', color);
    }
	...
	private drawCanvas(qr:qrcodegen.QrCode, scale:number, border:number, lightColor:string, darkColor:string, color:boolean):HTMLCanvasElement
    {
		...
		for (let y = -border; y < qr.size + border; y++) 
        {
			for (let x = -border; x < qr.size + border; x++) 
            {
                ...
                const custom = qr.getCustom(x, y);
                if (custom)
                {
                    if (custom.isFinderPattern && isDark) context.fillStyle = color ? '#9e4244' : '#000000';
                    if (custom.isAlignmentPattern && isDark) context.fillStyle = color ? '#3e4244' : '#000000';
                }
...
{	
    ...
    class Custom
    {
        public isFinderPattern:boolean = false;
        public isAlignmentPattern:boolean = false;
    }
	...
	export class QrCode 
    {
        ...
        {	
			...
			for (version = minVersion; ; version++) 
            {
				...
				if (version >= maxVersion) throw new RangeError("Data too long"); // All versions in the range could not fit the given data
			}
        ...
        // Testing... John
		private readonly custom:Array<Array<Custom>> = [];
        ...
        {	
			...
			for (let i = 0; i < this.size; i++) 
            {
                ...
                let r:Array<Custom> = [];
                this.custom.push(r);
                for (let j = 0; j < this.size; j++) r.push(new Custom());
			}
        ...
        public getCustom(x: int, y: int): Custom | undefined 
        {
            if (!this.custom[y]) return undefined;
			return this.custom[y][x];
		}
			...
			for (let dy = -4; dy <= 4; dy++) 
            {
				for (let dx = -4; dx <= 4; dx++) 
                {
					...
					if (0 <= xx && xx < this.size && 0 <= yy && yy < this.size)
                    {
                        ...
                        this.custom[yy][xx].isFinderPattern = true;
                    }
				}
			}
		}
		...
		private drawAlignmentPattern(x: int, y: int): void 
        {
			for (let dy = -2; dy <= 2; dy++) 
            {
				for (let dx = -2; dx <= 2; dx++)
                {
                    ...
                    this.custom[y + dy][x + dx].isAlignmentPattern = true;
                }
			}
		}
	...
	function appendBits(val: int, len: int, bb: Array<bit>): void 
    {
		...
		for (let i = len - 1; i >= 0; i--) bb.push((val >>> i) & 1); // Append bit by bit
	}
    ...
    {	
		...
		public static makeAlphanumeric(text: string): QrSegment 
        {
			...
			if (i < text.length) appendBits(QrSegment.ALPHANUMERIC_CHARSET.indexOf(text.charAt(i)), 6, bb); // 1 character remaining
		...
		public constructor(public readonly mode: QrSegment.Mode, public readonly numChars: int, private readonly bitData: Array<bit>) 
        {	
